## Setup:
* Kubernetes Clusters (i.e. Dev and Prod) running on AWS EKS. You can use eksctl to create clusters on AWS EKS. Use the        below command
```bash
eksctl create cluster --name <clusterName> --version 1.14 --region us-east-2 --nodegroup-name standard-workers3 --node-type t2.small --nodes 3 --nodes-min 1 --nodes-max 4
```
